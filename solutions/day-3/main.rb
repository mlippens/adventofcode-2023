...
puzzle_matrix.each_with_index do |line, line_number|
  ...
  line.each_with_index do |char, i|
    ...
    next unless (!is_num_char || i == line.length - 1) && parsed_num != ''
    ...
    puts "336: i=#{i} end_index=#{num_end_index} #{line.length}" if candidate == 336
    if num_end_index < line.length && symbol?(line[num_end_index + 1])
      adjacent_symbol = true
